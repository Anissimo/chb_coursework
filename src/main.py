...
def send_welcome(message):
    bot.send_message(message.chat.id, "Привет! О каком театре ты хотел бы узнать?")
    ...
    if matches:
        for match in matches:
            try:
                ...
                bot.send_message(message.chat.id, f"{match[1]['theaterName']}\n", reply_markup=markup)
...
def callback_query(call):
    ...
    elif data[0] == "another_theater":
        bot.send_message(call.message.chat.id, "Пожалуйста, введите название другого театра, который вы хотите исследовать.")
